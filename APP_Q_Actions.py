...
def apply_rule_local(step1_results, rule):
    ...
    df = pd.DataFrame(rows, columns=[
        "#","RepeaterName / Affiliates Name","RepeaterName / Connected from",
        "Site code","Traffic Area","DataCenter","Action Type",
    ...
    filename = f"Sales order {datetime.now().strftime('%d-%m-%Y')}.xlsx"
        ...
        if action == "update_db":
            ...
            if file and file.filename.endswith(".xlsx"):
                try:
                    ...
                    save_to_sqlite(df)
                    session.pop("step1_results", None)
                    session.pop("total_points", None)
                    session.pop("show_special_note", None)
                    message = "✅ DB updated"
            ...
            if data and rule:
                ...
                if filtered:
                    ...
                    STEP1_CACHE.pop(session.get("data_key"), None)
