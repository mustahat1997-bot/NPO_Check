...
def get_actions(points_text):
    ...
    for point in points_list:
        ...
        # 🔥 تحديد إذا يحتاج special note
        special_note = province.strip().lower() in ["basrah", "najaf"]

        # 🔥 إذا البوينت ما موجود بالشيت
        if repeater == "No Repeater":
            results.append({
                ...
                "r_action": None,
                "not_found": True,
                "special_note": False   # مهم حتى ما يصير error بالـ HTML
            })
            continue

        # 1️⃣ جرب البوينت بالـ DB أولاً
        q_action_db, r_action_db = get_actions_from_db(cursor, point)

        if (q_action_db and q_action_db.strip().lower() != "no action") or \
           (r_action_db and r_action_db.strip().lower() != "no action"):

            q_action = q_action_db
            r_action = r_action_db

        else:
            # 2️⃣ جرب السايت كود إذا موجود
            site_code = extract_site_code(repeater)

            if site_code:
                r_action_site = get_q_action_by_site_code(cursor, site_code)

                if r_action_site:
                    q_action = None
                    r_action = r_action_site
                else:
                    q_action = None
                    r_action = "Default => (+80%) International & Local CDN 100%"
            else:
                # 3️⃣ لا Site Code موجود
                q_action = None
                r_action = "Default => (+80%) International & Local CDN 100%"
        ...
        results.append({
            ...
            "r_action": r_action,
            "not_found": False,
            "special_note": special_note   # 🔥 هذا الجديد
        })
